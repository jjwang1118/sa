...
//  ------------------------------修改-----------------------------------------
const steps = ref(12);//12我不確定需不需要動 但小於12我們的圖繪變成很奇怪的
//  ------------------------------修改-----------------------------------------
...
const parsedLabels = computed(() => {
	...
	for (let i = 0; i < toParse.length; i++) {
		const label = toParse[i].x;
		const index = label.indexOf("T");

		if (index !== -1) {
			output.push(label.slice(index + 1));
		} else {
			output.push(label);
		}
	}
	//  ------------------------------修改-----------------------------------------
	if (output.some(label => label.includes("2022T"))) {
		return output.filter(label => !label.includes("2022T"));
	} else {
		let limitedOutput = [];
		for (let i = 0; i < steps.value; i++) {
			limitedOutput.push(output[i]);
		}
		limitedOutput.push("其他");
